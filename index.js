...
function findSixNewStories(html) {
  const topStories = [];
  const startMarker =
  ...
  for (let i = 0; i < 6; i++) {
    const item = items[i];
// extracting
    const titleStartIndex = item.indexOf(
      '<h3 class="latest-stories__item-headline">'
    );
    const titleEndIndex = item.indexOf("</h3>", titleStartIndex);
    const title = item
      .substring(
        titleStartIndex + '<h3 class="latest-stories__item-headline">'.length,
        titleEndIndex
      )
      .trim();
    ...
    const link = item.substring(
      linkStartIndex + '<a href="'.length,
      linkEndIndex
    );
  ...
  if (req.url === "/getTimeStories" && req.method === "GET") {
    ...
    https
      .get(url, (response) => {
        ...
        response.on("end", () => {
          const newStories = findSixNewStories(data);
          res.writeHead(200, { "Content-Type": "application/json" });
          const myNews = JSON.stringify(newStories);
          res.end(myNews);
        });
      })
      .on("error", (error) => {
